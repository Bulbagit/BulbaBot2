...
export async function execute(message) {
  ...
  if (linkRegex.test(message.content.toLowerCase())) {
    ...
    const url = `https://bulbapedia.bulbagarden.net/w/api.php?action=opensearch&search=${encodeURI(
      searchText.replace(/ /g, "_")
    ...
    try {
      ...
      if (response.statusCode === 200) {
        ...
        return message.reply(
          "Failed to fetch data from Bulbapedia. Please contact @BulbaTech!"
        );
      }
    } catch (error) {
      console.error(error);
      return message.reply(
        "An error occurred while fetching data. Please contact @BulbaTech!"
      );
    }
  }
  ...
  if (
    ...
    try {
      ...
      const response = new EmbedBuilder()
        ...
        .setDescription(
          `I was unable to remove message ${message.url}, in which I detected a Discord Link.`
        )
        .addFields([
          {
            name: "Message",
...
export async function filterMessage(message) {
  ...
  filters.forEach((filter) => {
    ...
    if (options) options = options.split(","); // ["minimumaccountage:3d", "warntime:5d", ...]
      ...
      Array.from(flags).forEach(async (flag) => {
        switch (flag) {
          ...
          case "k":
            ...
            await message.author.send({
              content:
                `You have been automatically kicked from ${message.guild.name} for your message in ${message.channel.name}, which is as follows:\n` +
          ...
          case "w":
            ...
            await ModLogs.create({
              ...
              .then(async () => {
                if (!banned && !kicked)
                  await message.author.send(
                    `Your message in the server ${message.guild.name}, in the channel ${message.channel.name}, was flagged by our system as being inappropriate.` +
            ...
            message
              ...
              .then(() => {
                if (!banned && !warned && !kicked)
                  message.author.send({
                    content:
                      `Your message in the server ${message.guild.name}, in the channel ${message.channel.name} has been identified as violating server rules and` +
          ...
          case "s":
            ...
            const warnings = await ModLogs.count({
              where: {
                loggerID: message.client.user.id,
                logTime: {
                  [Op.gte]: literal("DATE_SUB(NOW(), " + interval + ")"),
                },
                logName: "filter" + filterID,
              },
            });
