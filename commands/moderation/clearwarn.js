...
export async function execute(interaction) {
  ...
  const user = await interaction.client.users
    ...
    .catch((err) => {
      console.log(
        `Error fetching user; here's the loggedID:\n${warning.getDataValue(
          "loggedID"
        )}`
      );
      console.log(err);
      ...
      return interaction.reply(
        "Unable to fetch users. This may indicate a bug," +
  ...
  const mod = await interaction.client.users
    ...
    .catch((err) => {
      console.log(
        `Error fetching mod; here's the loggerID:\n${warning.getDataValue(
          "loggerID"
        )}`
      );
      console.log(err);
      ...
      warning
        ...
        .then(async () => {
          const response = new EmbedBuilder()
            ...
            .addFields(fields)
            .setDescription(
              `Warning ID#${warnID} deleted by ${interaction.user.username}.`
            )
            .setTimestamp();
        ...
        .catch(async (err) => {
          ...
          await i.followUp({
            content:
              "Failed to delete warning. Please inform the bot's administrator.",
          });
        });
    } else
      await i.update({
