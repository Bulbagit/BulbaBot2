    ...
    add
      ...
      .addStringOption((flags) =>
        flags
          .setName("flags")
          .setDescription(
            "Flags to set for the new filter. Refer to /filter help for a list of available flags and syntax."
          )
          .setRequired(true)
      )
      .addStringOption((options) =>
        options
          .setName("options")
          .setDescription(
            "Options for the new users and softban flags. Refer to /filter help for additional information."
          )
      )
  )
    ...
    edit
      ...
      .addStringOption((flags) =>
        flags
          .setName("flags")
          .setDescription(
            "Flags for the filter. Refer to /filter help for a list of available flags and syntax."
          )
      )
      .addStringOption((options) =>
        options
...
export async function execute(interaction) {
  ...
  switch (subcommand) {
    case "help": {
      const flagFields = [
        ...
        },
        {
          name: "n",
          value:
            "Only apply this filter to new users (either fresh accounts, those who have only recently joined the server, or both).",
        },
      ];
      ...
      const flagsEmbed = new EmbedBuilder()
        .setTitle("Flags")
        .setDescription(
          "List of available flags and their uses. The syntax is a comma-separated list without spaces."
        )
      ...
      const optionsEmbed = new EmbedBuilder()
        .setTitle("Options")
        .setDescription(
          "List of available options, their associated flags, and syntax. Enter them as a comma-separated list without spaces."
        )
    ...
    case "list": {
      ...
      if (filters.length === 0) return interaction.reply("No filters enabled.");
    ...
    case "add": {
      ...
      if (!valid)
        return interaction.reply(
          "Invalid flags specified. For more information, check /filter help for a list of valid flags and syntax."
        );
      ...
      if (options) {
        ...
        if (validOptions[1].length !== 0)
          return interaction.reply(
            "Invalid options or format. Please check /filter help for more information on valid options and syntax." +
      ...
      const row = new ActionRowBuilder().addComponents(addCancel, addConfirm);

      const addResponse = await interaction.reply({
        content: `Creating filter for term "${term}" with flags "${finalFlags}" and options "${finalOptions}"\nPlease confirm:`,
        components: [row],
      });
      ...
      try {
        const addConfirmation = await addResponse.awaitMessageComponent({
          filter: addCollectorFilter,
          time: 60000,
        });
        ...
        if (addConfirmation.customId === "confirm") {
          await Blacklist.create({
            ...
            .catch((err) => {
              ...
              return interaction.editReply({
                content:
                  "There was an error creating the filter. Please inform the bot's administrator.",
                components: [],
              });
            });
        } else
    ...
    case "remove": {
      ...
      if (!removeFilter)
        return interaction.reply({
          content: `No filter found with ID ${removeID}. You can confirm current filters with /filter list.`,
        });
      ...
      try {
        ...
        if (removeConfirmation.customId === "confirm") {
          Blacklist.destroy({ where: { id: removeID } })
            .then(() => {
              return interaction.editReply({
                content: "Filter successfully deleted.",
                components: [],
              });
            })
            .catch((err) => {
              ...
              return interaction.editReply({
                content:
                  "There was an error deleting the filter. Please inform the bot's administrator.",
                components: [],
              });
            });
        }
    ...
    case "edit": {
      ...
      if (!filter)
        return interaction.reply(
          `No filter found with ID ${filterID}. You can view current available filters with /filter list.`
        );
      ...
      if (!editTerm && !editFlags && !editOptions)
        return interaction.reply(
          "Nothing specified to edit. Please either supply a new term, new flags, or new options."
        );

      if (!editTerm) editTerm = filter.getDataValue("term");
      ...
      filter
        .update({
          term: editTerm,
          flags: finalEditFlags,
          options: finalEditOptions,
        })
        .then(async () => {
          ...
          const fields = [
            ...
            {
              name: "Options",
              value: finalEditOptions ? finalEditOptions : "none",
            },
...
export function validateFlags(flags, options) {
  ...
  flags.forEach((flag) => {
    switch (flag.toLowerCase()) {
      case "s":
        if (!optionsArray["warnlimit"]) options.push("warnlimit:3");
        if (!optionsArray["warntime"]) options.push("warntime:0s");
  ...
  if (flags.indexOf("s") !== -1 && flags.indexOf("w") === -1) flags.push("w"); // Softban should always warn
